...
public class BlogApiWebClient : IBlogApi
{
    ...
    //<Chapter7 SaveDeleteBlogPostCalls>
    ...
    public async Task DeleteBlogPostAsync(BlogPost item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Authenticated");
            await httpclient.DeleteAsJsonAsync<BlogPost>("api/BlogPosts", item);
        }
        catch (AccessTokenNotAvailableException exception)
        {
            exception.Redirect();
    ...
    public async Task DeleteCategoryAsync(Category item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Authenticated");
            await httpclient.DeleteAsJsonAsync<Category>("api/Categories", item);
        }
        catch (AccessTokenNotAvailableException exception)
        {
            exception.Redirect();
    ...
    public async Task DeleteTagAsync(Tag item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Authenticated");
            await httpclient.DeleteAsJsonAsync<Tag>("api/Tags", item);
        }
        catch (AccessTokenNotAvailableException exception)
        {
            exception.Redirect();
